...
void arbiter_logic(int requests[N], int grants[N]) {
    ...
    // Start searching at (last_granted + 1) to avoid fixed priority
    for (int offset = 1; offset <= N; offset++) {
        int current_idx = (last_granted_index + offset) % N;

        if (requests[current_idx]) {
            grants[current_idx] = 1;  
            last_granted_index = current_idx;
...
int main() {
    ...
    for (int cycle = 1; cycle <= MAX_CYCLES; cycle++) {
        ...
        // D. VISUALIZATION (Print first 100 cycles only to avoid clutter)
        if (cycle <= 100) {
